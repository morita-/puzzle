...
public class ExcelImporterEx : AssetPostprocessor
{
	class ExcelAssetInfoEx
	{
	...
	static ExcelAssetInfoEx FindAssetTypeByExcelName(string excelName)
	{
	...
	static void ImportExcel(string excelPath, ExcelAssetInfoEx info)
	{
		string assetPath = "";
        string assetOutputPath = "../Resources/ParameterAsset";
		...
		if(string.IsNullOrEmpty(info.Attribute.AssetPath))
		{
            ...
            assetPath = Path.Combine(basePath, assetOutputPath);
		}else{
			var path = Path.Combine("Assets", info.Attribute.AssetPath);
            assetPath = Path.Combine(path, assetOutputPath);
		}
